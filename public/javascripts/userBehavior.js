...
$(EventManager).bind("user-behavior-added", function(){
   if(userBehavior.length > 5){
       ...
       userBehavior = [];
       userBehavior.push({userID: globalUserId, name: globalname, time: (date.getDate()<10?'0':'') + date.getDate()  + " @ " + (date.getHours()<10?'0':'') + date.getHours()  + ":" + (date.getMinutes()<10?'0':'') + date.getMinutes()  + ":" + (date.getSeconds()<10?'0':'') + date.getSeconds() });
