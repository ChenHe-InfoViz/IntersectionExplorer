...
//         rowScale;
    ...
    globalStatistics: [
        ...
        {name: "universal set", id: "U", value: 200 }
    ],
        ...
        ctx.intersectionClicked = function (d) {
            ...
            if(ids.length == 1 && ids[0] == "Query"){
                ...
                ids.forEach(function (id) {
                    if (id == 0) {
                        descriptionText = "Papers related to none of the selected sets.";
                        ...
                        if(count == 0) {
                            switch(setIdToSet[id].cluster)
                            {
                                case 0:
                                    ...
                                    descriptionText = "Papers recommended by " + agentName;
                                    break;
                                case 1:
                                    userName = names[0].trim() + ", ";
                                    descriptionText = "Papers bookmarked by " + userName;
                                    break;
                                case 2:
                                    tagName = names[0].trim() + ", ";
                                    descriptionText = "Papers tagged with " + tagName;
                                    break;
                            }
                ...
                if(ids.length > 1) {
                    descriptionText = "Papers ";
                    if (agentName != "")
                        descriptionText += "recommended by " + agentName;
                    if (userName != "")
                        descriptionText += "bookmarked by " + userName;
                    if (tagName != "")
                        descriptionText += "tagged with " + tagName;
                    if (usedSets.length > ids.length) {
                        descriptionText += "but not related to the rest of the selected sets.";
                    }
                    else {
                        descriptionText = descriptionText.slice(0, -2);
                        descriptionText += ".";
...
//chen: logicPanel: the query setting panel
        ...
        //ctx.sortNode = ctx.svgHeader.append("g").attr({
        //        class: "sortPart",
        //        width: "55",
        //        height: "16",
        //        "transform": "translate("+ctx.xStartSetSizes+", 0)"
        //});
...
//console.log("tableHeaderNode: " + ctx.tableHeaderNode.attr("width"));
...
//            })
    ...
    function updateSortBy(svgHeaderNode) {
        ...
        div.append("div").text("number of papers");
    ...
    function updateSetsLabels(tableHeaderNode) {
        ...
        .on('mouseout', function(d,i){
            ...
            .text(function (d, i) {
                if(d.id == globalSetId)
                    return "Papers bookmarked by you.";
                var names = d.elementName.split(/\(|\)|\//g);

                switch (d.cluster){
                    case 0:
                        return names[1].trim() + " papers recommended by " + names[0].trim() + ".";
                    case 1:
                        if(parseInt(names[2].trim()) > 1)
                            return names[2].trim() + " papers bookmarked by " + names[0].trim() + ".";
                        else
                            return names[2].trim() + " paper bookmarked by " + names[0].trim() + ".";
                    case 2:
                        if(parseInt(names[1].trim()) > 1)
                            return names[1].trim() + " papers tagged with " + names[0].trim() + ".";
                        else
                            return names[1].trim() + " paper tagged with " + names[0].trim() + ".";
                }
                return "";
            })
            .style({
        ...
        d3.selectAll('.sortBySet, .setRow .setLabel').on(
            ...
            function (d, i) {
                ...
                $("#element-view-description").text("All papers " + text + ".").css("color", selections.getColor(selection));
                document.getElementById("tipbar").innerHTML = "Showing all papers " + text + " on the right side.";
      ...
      var rowSubSets = subSets
        ...
        .on({
            'mouseenter': function(d, i) {
                ...
                if(sets.length == 1){
                    if(sets[0] == "") {
                        document.getElementById("tipbar").innerHTML = "Click to view papers related to none of the selected sets on the right side.";
                    }
                    else {
                        var names = setIdToSet[sets[0]].elementName.split(/\(|\)/g);
                        document.getElementById("tipbar").innerHTML = "Click to view papers only related to " + names[0].trim() + " on the right side.";
                    }
                }
                else{
                    var ids = d.data.elementName.split(",");
...
//        });
        ...
        $(EventManager).bind("refresh", function(event, data){
            // paper id: data has been re-coded
            var paperIDfromCN = attributes[0].values[data];
            //console.log(paperIDfromCN);
            for(i = 0; i < dataForVis.UserAndBookmarks.length; i++)
            {
                if(dataForVis.UserAndBookmarks[i]['bookmarks'].indexOf(paperIDfromCN) > -1)
                {
                    //add common bookmarks count
                    var tempID = dataForVis.UserAndBookmarks[i]['setID'];
                    var parts = setIdToSet[tempID].elementName.split(/\(|\)|\//g);
                    //console.log(parts);
                    parts[1] = (++dataForVis.commonCount[dataForVis.UserAndBookmarks[i]['userID']]);
                                ...
                                switch (d.cluster){
                                    case 1:
                                        if(parseInt(parts[2].trim()) > 1)
                                            return parts[2].trim() + " papers bookmarked by " + parts[0].trim() + ".";
                                        else
                                            return parts[2].trim() + " paper bookmarked by " + parts[0].trim() + ".";
                                }
                                return "";
                            })
                    }
            ...
            var newStr = " " + names[0] + "(" + (parseInt(names[1], 10) + 1) + "/" + (parseInt(names[2], 10)+ 1) + ")";
...
//                UpSetState.levelTwoGrouping = undefined;
                ...
                updateState();
                rowTransition();
                document.getElementById("tipbar").innerHTML = "Sort rows by the number of attended sets (ascending)."
            });
...
//                UpSetState.levelTwoGrouping = undefined;
                ...
                updateState();
                rowTransition();
                document.getElementById("tipbar").innerHTML = "Sort rows by the number of attended sets (descending)."
            });
        ...
        d3.selectAll('#sortIntersectionSize').on(
            ...
            function (d) {
                ...
                updateState();
                rowTransition();
                document.getElementById("tipbar").innerHTML = "Sort rows by their related number of papers (largest to smallest)."
            });
        ...
        d3.selectAll('#sortIntersectionSizeAscend').on(
            ...
            function (d) {
                ...
                updateState();
                rowTransition();
                document.getElementById("tipbar").innerHTML = "Sort rows by their related number of papers (smallest to largest)."
            });
